...
- loader
  ...
  由于 webpack 原生只支持 js 和 JSON 两种类型的文件，因此需要对其他类型的文件进行转换，进行转换的工具就是 loader，loader 可以链式调用，从右到左的顺序调用，前一个 loader 的返回结果会传递给下一个 loader。
  loader 的配置方式如下：
  ...
  module.exports = {
    module: {
      rules: [
        {
          test: /\.css$/, //匹配需要应用loader的文件
          use: [ // 多个loader通过设置use属性来配置
          'style-loader',
          {
             loader: 'css-loader',
             options: {}//选项信息
        ...
        {
          test: /\.jsx$/,
          loader: 'babel-loader',//一个loader可以直接设置loader属性
